<!-- resources/js/pages/admin/inspectorate/assigned.vue -->
<script setup>
...
const statusOptions = [
    ...
    { label: 'Pending Review', value: 'ec_approved' },
    { label: 'Approved', value: 'inspectorate_approved' },
    { label: 'Rejected', value: 'sent_back' },
    { label: 'TCO Pending', value: 'inspectorate_approved' },
];
...
const loadVouchers = async () => {
    ...
    try {
        ...
        const response = await axios.get('/inspectorate/assigned/search', { params });
...
const exportToExcel = () => {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Inspectorate_Assigned_Vouchers');
        XLSX.writeFile(wb, `Inspectorate_Assigned_Vouchers_${new Date().toISOString().split('T')[0]}.xlsx`);
...
const exportToPDF = () => {
    ...
    try {
        ...
        doc.text('Inspectorate Assigned Vouchers', pageWidth / 2, 15, { align: 'center' });
        ...
        doc.save(`Inspectorate_Assigned_Vouchers_${new Date().toISOString().split('T')[0]}.pdf`);
...
const handleApprove = () => {
    ...
    router.post(`/inspectorate/assigned/${currentVoucher.value.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: (response) => {
...
const handleReject = () => {
    ...
    router.post(`/inspectorate/assigned/${currentVoucher.value.id}/reject`, {
...
const closeSuccessModal = () => {
    ...
    setTimeout(() => {
        router.visit('/inspectorate/assigned');
    }, 300);
};
...
onMounted(() => {
    ...
    console.log('Assigned Vouchers Props:', props);
    console.log('Vouchers data:', props.vouchers);
    console.log('Stats data:', props.stats);
    ...
    // Check if vouchers data exists and has data
    if (props.vouchers && props.vouchers.data && props.vouchers.data.length > 0) {
        ...
        console.log('Loaded vouchers from props:', vouchers.value.length);
    } else {
        console.log('No initial vouchers, loading...');
        loadVouchers();
    }
});
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Inspectorate Assigned Vouchers" />
        <Toast />
        ...
        <div class="mb-4">
            <Message severity="info" :closable="false" class="workflow-banner">
                <div class="flex align-items-center gap-3 flex-wrap">
                    ...
                    <div>
                        <strong>Inspectorate Assigned Vouchers</strong>
                        <div class="text-sm mt-1">
                            These are Salary and Pension vouchers assigned to you by Inspectorate Admin.
                            You can review, approve, or reject them.
                        </div>
                    </div>
                </div>
            </Message>
        ...
        <Card class="main-card">
            <template #title>
                <div class="flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="flex align-items-center gap-2">
                        ...
                        <span>Inspectorate Assigned Vouchers</span>
            ...
            <template #content>
                ...
                <DataTable
                    ...
                    :emptyMessage="activeTab === 'pending' ? 'No pending vouchers assigned to you.' : activeTab === 'approved' ? 'No approved vouchers.' : activeTab === 'rejected' ? 'No rejected vouchers.' : 'No vouchers assigned to you.'"
                    :paginator="true"

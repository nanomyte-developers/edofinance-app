<!-- resources/js/pages/admin/inspectorate/show.vue -->
<script setup>
...
// Check if voucher is ready for Inspectorate approval
...
const openApproveModal = () => {
    if (!isReadyForApproval.value) {
        ...
        errorMessage.value = `This voucher has status "${getStatusDisplayName(props.voucher.status)}" and must be "Forwarded from EC" before Inspectorate can process it.`;
...
const openRejectModal = () => {
    if (!isReadyForApproval.value) {
        ...
        errorMessage.value = `This voucher has status "${getStatusDisplayName(props.voucher.status)}" and must be "Forwarded from EC" before Inspectorate can reject it.`;
        showErrorModal.value = true;
        return;
    }
...
const handleApprove = () => {
    ...
    router.post(`/inspectorate/vouchers/${props.voucher.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: (response) => {
            ...
            successMessage.value = response.message || `Voucher has been successfully forwarded to Treasury Cash Office (TCO).`;
...
const handleReject = () => {
    ...
    router.post(`/inspectorate/vouchers/${props.voucher.id}/reject`, {
        reason: rejectionReason.value,
    }, {
        preserveScroll: true,
        onSuccess: () => {
            successTitle.value = 'Voucher Rejected';
            successMessage.value = `Voucher ${props.voucher.voucher_number} has been rejected and returned to Expenditure Control.`;
            successDetails.value = {
                voucher_number: props.voucher.voucher_number,
                amount: formatCurrency(props.voucher.total_amount),
                reason: rejectionReason.value,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="`Inspectorate - ${voucher.voucher_number}`" />
        <Toast />
        ...
        <div class="mb-4 flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="flex align-items-center gap-3">
                ...
                <div class="flex align-items-center gap-2">
                    ...
                    <h1 class="text-2xl font-semibold m-0">Inspectorate Review</h1>
                </div>
            </div>
        ...
        <div v-if="!isReadyForApproval" class="mb-4">
            <Message severity="warn" :closable="false">
                <div class="flex align-items-center gap-2">
                    ...
                    <div>
                        <strong>Voucher Not Ready for Inspectorate Review</strong>
                        <div class="text-sm mt-1">
                            This voucher has status <strong>{{ getStatusDisplayName(voucher.status) }}</strong>. 
                            Only vouchers with status <strong>"Forwarded from EC"</strong> can be processed by Inspectorate.
                        </div>
                    </div>
                </div>
            </Message>
                ...
                <div class="flex align-items-center gap-3 flex-wrap">
                    ...
                    <div>
                        ...
                        <div class="text-sm mt-1">
                            ...
                            Once approved, this voucher will go to <strong>Treasury Cash Office (TCO)</strong> for final processing.
                        </div>
                    </div>
                </div>
            </Message>
        ...
        <div class="grid">
            ...
            <div class="col-12 lg:col-8">
                ...
                <Card class="mt-4 line-items-card">
                    ...
                    <template #content>
                        <DataTable :value="voucher.items || []" class="p-datatable-sm" responsiveLayout="scroll">
                    ...
                    <template #content>
                        <div class="flex flex-column gap-3">
                            ...
                            <div class="flex justify-content-between align-items-center">
                                ...
                                <Tag value="Inspectorate" severity="warning" />
                            </div>
                            <div class="flex justify-content-between align-items-center">
                                <span class="text-500">Required Status:</span>
                                <Tag value="Forwarded from EC" severity="warning" icon="pi pi-arrow-right" />
                            </div>
                            <div class="flex justify-content-between align-items-center">
                            ...
                            <div class="flex justify-content-between align-items-center">
                                ...
                                <Tag value="Treasury Cash Office (TCO)" severity="info" icon="pi pi-arrow-right" />
                            </div>
                        </div>
                    </template>
                </Card>
            ...
            <div class="flex flex-column gap-3">
                ...
                <div class="border-round bg-yellow-50 p-3">
                    ...
                    <div class="text-sm text-600">
                        This voucher will be forwarded to Treasury Cash Office for final processing.
        ...
        >
            <div class="flex flex-column gap-3">
                <div class="flex align-items-center gap-3 p-3 bg-red-50 border-round">
                    ...
                    <div>
                        <div class="font-semibold">Voucher: {{ voucher.voucher_number }}</div>
                        <div class="text-sm">This action will return the voucher to Expenditure Control.</div>
                    </div>
                </div>
            ...
            <div class="flex flex-column align-items-center text-center">
                ...
                <div class="bg-yellow-50 p-3 border-round w-full text-left">
                    ...
                    <ul class="m-0 pl-3 text-sm">
                        <li>Ensure the voucher has been approved by Expenditure Control</li>
                        <li>The voucher status should be "Forwarded from EC"</li>
                        <li>Contact Expenditure Control if the voucher needs to be reviewed</li>

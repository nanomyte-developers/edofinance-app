<!-- resources/js/pages/admin/inspectorate/index.vue -->
<script setup>
...
const statusOptions = [
    ...
    { label: 'Pending EC Review', value: 'forwarded' },
    { label: 'Pending Inspectorate Review', value: 'ec_approved' },
    { label: 'Inspectorate Approved', value: 'inspectorate_approved' },
...
const breadcrumbs = [
    { title: 'Inspectorate', href: '/inspectorate' },
    { title: 'Queue', href: '#' },
];
...
const getStatusDisplayName = (status) => {
    const names = {
        ...
        ec_approved: 'Pending Inspectorate Review',
        inspectorate_approved: 'Inspectorate Approved',
        sent_back: 'Sent Back',
        rejected: 'Rejected',
        tco_pending: 'Pending TCO',
...
// Check if voucher is ready for Inspectorate approval (EC approved)
...
const loadVouchers = async () => {
    ...
    try {
        ...
        const response = await axios.get('/inspectorate/search', { params });
...
const exportToExcel = () => {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Inspectorate_Vouchers');
        XLSX.writeFile(wb, `Inspectorate_Review_Vouchers_${new Date().toISOString().split('T')[0]}.xlsx`);
...
const exportToPDF = () => {
    ...
    try {
        ...
        doc.text('Inspectorate Review - Vouchers', pageWidth / 2, 15, { align: 'center' });
        ...
        autoTable(doc, {
            head: [['Voucher #', 'Type', 'Date', 'EC Approved', 'MDA', 'Payee', 'Amount', 'Status', 'Payment Status']],
            body: tableData,
            startY: 35,
            theme: 'striped',
            headStyles: {
        ...
        doc.save(`Inspectorate_Review_Vouchers_${new Date().toISOString().split('T')[0]}.pdf`);
...
const openApproveModal = (voucher) => {
    if (!isReadyForApproval(voucher)) {
        ...
        errorMessage.value = `This voucher has status "${getStatusDisplayName(voucher.status)}" and must be "Pending Inspectorate Review" before you can process it.`;
...
const openRejectModal = (voucher) => {
    if (!isReadyForApproval(voucher)) {
        ...
        errorMessage.value = `This voucher has status "${getStatusDisplayName(voucher.status)}" and must be "Pending Inspectorate Review" before you can reject it.`;
        showErrorModal.value = true;
        return;
    }
    currentVoucher.value = voucher;
...
// Handle Inspectorate approval (forwards to TCO)
const handleApprove = () => {
    isProcessing.value = true;

    router.post(`/inspectorate/vouchers/${currentVoucher.value.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: (response) => {
            ...
            successMessage.value = response.message || `Voucher has been successfully forwarded to TCO.`;
            successDetails.value = {
                ...
                forwarded_to: 'Treasury Cash Office (TCO)',
                status: 'Inspectorate Approved',
...
const handleReject = () => {
    ...
    router.post(`/inspectorate/vouchers/${currentVoucher.value.id}/reject`, {
...
const handleAssign = () => {
    ...
    router.post(`/inspectorate/vouchers/${currentVoucher.value.id}/assign`, {
        user_id: userId,
    }, {
        preserveScroll: true,
        onSuccess: () => {
...
const closeSuccessModal = () => {
    ...
    setTimeout(() => {
        router.visit('/inspectorate');
    }, 300);
};

// View voucher details
const viewVoucherDetails = (voucher) => {
    router.visit(`/inspectorate/vouchers/${voucher.id}`);
};
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Inspectorate Review" />
        <Toast />
        ...
        <div class="mb-4">
            <Message severity="info" :closable="false" class="workflow-banner">
                <div class="flex align-items-center gap-3 flex-wrap">
                    ...
                    <div>
                        <strong>Inspectorate - Salary & Pension Vouchers</strong>
                        <div class="text-sm mt-1">
                            Review Salary and Pension vouchers approved by Expenditure Control (EC).
                            Once approved, vouchers go to <strong>Treasury Cash Office (TCO)</strong> for final processing.
                        </div>
                    </div>
                </div>
            </Message>
        ...
        <div class="mb-4">
            <div class="flex align-items-center gap-3 flex-wrap">
                <Button
                    :label="`Vouchers from EC (${stats.pending_ec_count || 0})`"
                ...
                />
                <Button
                    :label="`Pending Inspectorate Review (${stats.pending_inspectorate_count || 0})`"
        ...
        <Card class="main-card">
            <template #title>
                <div class="flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="flex align-items-center gap-2">
                        ...
                        <span>{{ activeTab === 'pending' ? 'Pending Inspectorate Review' : activeTab === 'approved' ? 'Approved Vouchers' : 'Vouchers from EC' }}</span>
            ...
            <template #content>
                ...
                <DataTable
                    ...
                    :emptyMessage="activeTab === 'pending' ? 'No vouchers pending Inspectorate review.' : activeTab === 'approved' ? 'No approved vouchers found.' : 'No vouchers found.'"
                    :paginator="true"
                ...
                >
                    ...
                    <Column field="voucher_number" header="Voucher #" headerStyle="width: 10%" :sortable="true">
                        <template #body="slotProps">
                            <Link 
                                :href="`/inspectorate/vouchers/${slotProps.data.id}`" 
                    ...
                    <!-- Actions - 3 up, 3 down layout with ASSIGN BUTTON -->
                            ...
                            <div class="flex flex-column gap-1 align-items-center">
                                ...
                                <div class="flex gap-1 justify-content-center">
                                    ...
                                    <Button
                                        ...
                                        rounded
                                        size="small"
                                        v-tooltip.top="'Approve & Forward to TCO'"
        ...
        >
            <div class="flex flex-column gap-3">
                ...
                <div class="border-round bg-yellow-50 p-3">
                    <div class="flex align-items-center gap-2 mb-2">
                        ...
                        <Tag value="Treasury Cash Office (TCO)" severity="info" />
                    </div>
                    <div class="text-sm text-600">
                        This voucher will be forwarded to TCO for final processing.
            ...
            <template #footer>
                ...
                <Button label="Approve & Forward to TCO" icon="pi pi-send" severity="success" @click="handleApprove" :loading="isProcessing" />
            </template>
        </Dialog>

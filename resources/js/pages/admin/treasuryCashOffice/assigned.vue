<!-- resources/js/pages/admin/tco/assigned.vue -->
<script setup>
...
const statusOptions = [
    ...
    { label: 'Pending Review', value: 'inspectorate_approved' },
...
const loadVouchers = async () => {
    ...
    try {
        ...
        const response = await axios.get('/treasury-cash-office/assigned/search', { params });
...
const exportToExcel = () => {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'TCO_Assigned_Vouchers');
        XLSX.writeFile(wb, `TCO_Assigned_Vouchers_${new Date().toISOString().split('T')[0]}.xlsx`);
...
const exportToPDF = () => {
    ...
    try {
        ...
        doc.text('TCO Assigned Vouchers', pageWidth / 2, 15, { align: 'center' });
        ...
        doc.save(`TCO_Assigned_Vouchers_${new Date().toISOString().split('T')[0]}.pdf`);
...
const openApproveModal = (voucher) => {
    if (!isReadyForApproval(voucher)) {
        if (!voucher.bank_activity) {
            ...
            errorMessage.value = 'This voucher does not have a bank account assigned. Please contact TCO Admin to assign a bank.';
        } else {
            errorTitle.value = 'Cannot Mark as Paid';
...
const handleApprove = () => {
    ...
    router.post(`/treasury-cash-office/assigned/${currentVoucher.value.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: (response) => {
        ...
        onError: (errors) => {
            ...
            if (errors.response?.data?.message) {
                errorTitle.value = 'Payment Failed';
                ...
                toast.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'Failed to mark voucher as paid.',
                    life: 5000,
                });
            }
        },
...
const handleReject = () => {
    ...
    router.post(`/treasury-cash-office/assigned/${currentVoucher.value.id}/reject`, {
...
const closeSuccessModal = () => {
    ...
    setTimeout(() => {
        router.visit('/treasury-cash-office/assigned');
    }, 300);
};
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="TCO Assigned Vouchers" />
        <Toast />
        ...
        <div class="mb-4">
            <Message severity="info" :closable="false" class="workflow-banner">
                <div class="flex align-items-center gap-3 flex-wrap">
                    ...
                    <div>
                        <strong>TCO Assigned Vouchers</strong>
                        <div class="text-sm mt-1">
                            These are Salary and Pension vouchers assigned to you by Treasury Cash Office Admin.
                            You can review, mark as paid, or reject them.
                        </div>
                    </div>
                </div>
            </Message>
        ...
        <div class="mb-4">
            <div class="flex align-items-center gap-3 flex-wrap">
                ...
                <Button
                    :label="`Paid (${stats.approved_count || 0})`"
        ...
        <Card class="main-card">
            <template #title>
                <div class="flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="flex align-items-center gap-2">
                        ...
                        <span>TCO Assigned Vouchers</span>
            ...
            <template #content>
                ...
                <DataTable
                    ...
                    :emptyMessage="activeTab === 'pending' ? 'No pending vouchers assigned to you.' : activeTab === 'paid' ? 'No paid vouchers.' : activeTab === 'rejected' ? 'No rejected vouchers.' : 'No vouchers assigned to you.'"
                    :paginator="true"
                    ...
                    <Column header="Actions" headerStyle="width: 12%" bodyClass="text-center">
                        <template #body="slotProps">
                            <div class="flex flex-column gap-1 align-items-center">
                                ...
                                <div class="flex gap-1 justify-content-center">
                                    <Button
                                        v-if="slotProps.data.status === 'inspectorate_approved' && slotProps.data.bank_activity"
                                        icon="pi pi-check-circle"
                                        severity="success"
                                        text
                                        rounded
                                    ...
                                    />
                                    <Button
                                        v-else-if="slotProps.data.status === 'inspectorate_approved' && !slotProps.data.bank_activity"
                                        icon="pi pi-check-circle"
                                        severity="warning"
                                        text
                                        rounded
            ...
            <div class="flex flex-column gap-3">
                ...
                <div v-else class="border-round bg-red-50 p-3">
                    <div class="flex align-items-center gap-2">
                        ...
                        <span class="text-sm">No bank assigned. Please contact TCO Admin to assign a bank.</span>
                    </div>
                </div>

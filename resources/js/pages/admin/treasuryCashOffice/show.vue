<!-- resources/js/pages/admin/tco/show.vue -->
<script setup>
...
const getStatusDisplayName = (status) => {
    const names = {
        inspectorate_approved: 'Forwarded from Inspectorate',
        closed: 'Paid',
...
// Check if voucher is ready for TCO approval
const isReadyForApproval = computed(() => {
    return props.voucher.status?.toLowerCase() === 'inspectorate_approved' && props.voucher.bank_activity;
});

// Open approval modal
const openApproveModal = () => {
    if (!isReadyForApproval.value) {
        if (!props.voucher.bank_activity) {
            ...
            errorTitle.value = 'Cannot Mark as Paid';
            errorMessage.value = `This voucher has status "${getStatusDisplayName(props.voucher.status)}" and must be "Forwarded from Inspectorate" before Treasury Cash Office can process it.`;
        }
...
const openRejectModal = () => {
    if (!isReadyForApproval.value) {
        ...
        errorMessage.value = `This voucher has status "${getStatusDisplayName(props.voucher.status)}" and must be "Forwarded from Inspectorate" before Treasury Cash Office can reject it.`;
        showErrorModal.value = true;
        return;
    }
...
const handleApprove = () => {
    ...
    router.post(`/treasury-cash-office/vouchers/${props.voucher.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: (response) => {
            ...
            successMessage.value = response.message || `Voucher has been marked as paid successfully. The workflow is now complete.`;
...
const handleReject = () => {
    ...
    router.post(`/treasury-cash-office/vouchers/${props.voucher.id}/reject`, {
        reason: rejectionReason.value,
    }, {
        preserveScroll: true,
        onSuccess: () => {
            successTitle.value = 'Voucher Rejected';
            successMessage.value = `Voucher ${props.voucher.voucher_number} has been rejected and returned to Inspectorate.`;
            successDetails.value = {
                voucher_number: props.voucher.voucher_number,
                amount: formatCurrency(props.voucher.total_amount),
                reason: rejectionReason.value,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="`Treasury Cash Office - ${voucher.voucher_number}`" />
        <Toast />
        ...
        <div class="mb-4 flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="flex align-items-center gap-3">
                ...
                <div class="flex align-items-center gap-2">
                    ...
                    <h1 class="text-2xl font-semibold m-0">Treasury Cash Office Review</h1>
                </div>
            </div>
        ...
        <div v-if="!isReadyForApproval" class="mb-4">
            <Message severity="warn" :closable="false">
                <div class="flex align-items-center gap-2">
                    ...
                    <div>
                        <strong>Voucher Not Ready for Payment</strong>
                        <div class="text-sm mt-1">
                            This voucher has status <strong>{{ getStatusDisplayName(voucher.status) }}</strong>. 
                            Only vouchers with status <strong>"Forwarded from Inspectorate"</strong> and a bank account assigned can be marked as paid.
                        </div>
                    </div>
                </div>
            </Message>
            ...
            <Message severity="success" :closable="false" class="workflow-banner">
                <div class="flex align-items-center gap-3 flex-wrap">
                    ...
                    <div>
                        ...
                        <div class="text-sm mt-1">
                            Reviewing voucher <strong>{{ voucher.voucher_number }}</strong> for final payment.
                            Once marked as paid, the workflow is complete.
                        </div>
                    </div>
                </div>
            </Message>
                ...
                <Card class="voucher-details-card">
                    ...
                    <template #content>
                        <div class="grid">
                            ...
                            <div class="col-12">
                                <div class="field mb-3">
                                    <label class="text-500 text-sm font-semibold block mb-1">Payment Bank</label>
                    ...
                    <template #content>
                        <div class="flex flex-column gap-3">
                            ...
                            <div class="flex justify-content-between align-items-center">
                                ...
                                <Tag value="Treasury Cash Office (TCO)" severity="success" />
                            </div>
                            <div class="flex justify-content-between align-items-center">
                                <span class="text-500">Required Status:</span>
                                <Tag value="Forwarded from Inspectorate" severity="warning" icon="pi pi-arrow-right" />
                            </div>
                            <div class="flex justify-content-between align-items-center">
                            ...
                            <div class="flex justify-content-between align-items-center">
                                ...
                                <Tag value="Mark as Paid (Final Step)" severity="success" icon="pi pi-check-circle" />
                            </div>
                        </div>
                    </template>
                </Card>
                            ...
                            >
                                <template #marker="slotProps">
                                    <span 
                                        ...
                                        :class="{
                                            'bg-green-500': slotProps.item.action === 'Approved' || slotProps.item.action === 'Paid',
                                    ...
                                    >
                                        <i :class="{
                                            'pi pi-check': slotProps.item.action === 'Approved' || slotProps.item.action === 'Paid',
                                            ...
                                            <div class="flex flex-column gap-2">
                                                <div class="flex align-items-center justify-content-between flex-wrap">
                                                    ...
                                                    <Badge 
                                                        ...
                                                        :severity="slotProps.item.action === 'Approved' || slotProps.item.action === 'Paid' ? 'success' : 
        ...
        >
            ...
            <template #footer>
                ...
                <Button label="Mark as Paid" icon="pi pi-check-circle" severity="success" @click="handleApprove" :loading="isProcessing" :disabled="!voucher.bank_activity" />
            </template>
        </Dialog>
            ...
            <div class="flex flex-column align-items-center text-center">
                ...
                <div class="bg-yellow-50 p-3 border-round w-full text-left">
                    ...
                    <ul class="m-0 pl-3 text-sm">
                        <li>Ensure the voucher has been approved by Inspectorate</li>
                        <li>The voucher status should be "Forwarded from Inspectorate"</li>
                        <li>A bank account must be assigned to the voucher</li>
                        <li>Contact Inspectorate if the voucher needs to be reviewed</li>

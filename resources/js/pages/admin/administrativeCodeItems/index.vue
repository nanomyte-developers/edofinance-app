...
const itemDataArray = ref(props.administrativeCodeItems.data);
watch(
    () => props.administrativeCodeItems.data,
    (newData) => {
...
const itemData = computed(() => props.administrativeCodeItems);
...
const route = (name, params) => {
    // Updated to match your actual route names (administrative-code-items plural)
    if (
        name === 'administrative-code-itemss.store' ||
        name === 'administrative-code-itemss.index'
    ) {
        return '/administrative-code-itemss';
    }
    if (
        name === 'administrative-code-itemss.update' &&
        params &&
        params.administrative_code_item
    ) {
        const id = params.administrative_code_item;
        return `/administrative-code-itemss/${id}`;
    }
    if (
        name === 'administrative-code-itemss.destroy' &&
        params &&
        params.administrative_code_item
    ) {
        const id = params.administrative_code_item;
...
const breadcrumbs = [
    {
        title: 'Administrative Code Items Management',
        href: route('administrative-code-itemss.index'),
    },
];
...
const handleEditItem = (item) => {
    ...
    form.administrative_code_id = item.administrative_code_id;
    form.name = item.name;
    form.code = item.code;
...
const saveItem = () => {
    ...
    if (isEdit.value && currentItem.value) {
        // Use PUT for update
        form.put(
            '/administrative-code-itemss/' +
            currentItem.value.id,
            options,
        );
    } else {
        // Use POST for store
        // form.post(route('administrative-code-items.store'), options);
        form.post('/administrative-code-itemss', options);
    }
};
...
const confirmDeleteItem = (item) => {
    confirm.require({
        ...
        accept: () => {
            router.delete(
                route('administrative-code-itemss.destroy', {
                    administrative_code_item: item.id,
                }),
                {
                    preserveScroll: true,
                    onSuccess: () => {
...
const onPageChange = (event) => {
    ...
    // const pageUrl = itemData.value.links[event.page + 1];

    console.log(itemData.value.meta.links);
...
onMounted(() => {
    console.log(props.administrativeCodeItems);
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <Head title="Administrative Code Item Management" />
            ...
            <template #title>
                <div class="flex flex-col items-start justify-between p-3 sm:flex-row sm:items-center">
                    <h2 class="text-2xl font-extrabold text-gray-800">
                        Administrative Code Items List ({{
            ...
            <template #content>
                ...
                <div class="overflow-x-auto">
                    <DataTable :value="itemDataArray" dataKey="id" stripedRows responsiveLayout="scroll"
                        ...
                        :emptyMessage="'No Administrative Code Item found. Try creating a new one or adjusting your search.'">
                        ...
                        <Column field="administrative_code_name" header="Administrative Parent Code" headerStyle="width: 25%"
                            :sortable="true">
                            <template #body="slotProps">
                                <Tag :value="slotProps.data.administrative_code_name ||
                                    'N/A'
        ...
        <Dialog v-model:visible="showCreateEditModal" :style="{ width: '500px' }" :header="isEdit
                ? 'Edit Administrative Code Item'
                : 'Create New Administrative Code Item'
            " :modal="true" class="p-fluid" :closable="!form.processing">
            <form @submit.prevent="saveItem" class="mt-2 flex flex-col gap-4">
                <div class="field">
                    <label for="administrative_code_id" class="mb-2 block font-semibold">
                        Parent Administrative Code <span class="text-red-500">*</span>
                    </label>
                    <Dropdown id="administrative_code_id" v-model="form.administrative_code_id" :options="props.administrativeCodes"
                        optionLabel="name" optionValue="id" placeholder="Select Administrative Parent Code" class="w-full"
                        :class="{ 'p-invalid': form.errors.administrative_code_id }" :filter="true" :disabled="form.processing"
                        :showClear="true" />
                    <small v-if="form.errors.administrative_code_id" class="p-error mt-1 block">

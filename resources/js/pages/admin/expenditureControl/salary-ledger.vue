<template>
    <AppLayout>
        <Head title="Salary Expenditure Ledger" />
        ...
        <div class="card p-4 bg-white ledger-container">
            ...
            <div class="text-center mb-4 uppercase-header">
                ...
                <h2 class="font-bold mt-2 text-xl text-gray-800 tracking-wider">SALARY EXPENDITURE</h2>
            ...
            <div class="flex justify-content-between align-items-center mb-4 flex-wrap gap-3 no-print">
                <div>
                    ...
                    <span class="text-500 text-sm">
                        Showing only paid/closed salary vouchers
                    </span>
                </div>
            ...
            <div class="mb-5 grid no-print">
                ...
                <div class="col-12 md:col-3">
                    <div class="card surface-50 p-3">
                        ...
                        <div class="text-500 text-xs">Paid salary</div>
                    </div>
                </div>
            </div>

            <!-- Salary Type Breakdown - Hidden on Print -->
            <div v-if="salaryTypeStats.length" class="mb-4 no-print">
                <h5 class="mb-2">Salary Breakdown by Type</h5>
                <div class="grid">
                    <div v-for="stat in salaryTypeStats" :key="stat.type" class="col-12 md:col-3 lg:col-2">
                        <div class="card surface-50 p-2 text-center">
                            <div class="text-sm font-bold capitalize">{{ stat.type }}</div>
            ...
            <div class="overflow-x-auto">
                ...
                >
                    ...
                    <Column field="voucher_type" header="VOUCHER TYPE" style="min-width: 130px; width: 9%">
                        <template #body="slotProps">
                            <Tag 
                                :value="slotProps.data.voucher_type || 'N/A'" 
                                :severity="getSalaryTypeSeverity(slotProps.data.voucher_type)"
                                size="small"
                            />
                        </template>
                    </Column>

                    <Column field="pay_point" header="PAY POINT" style="min-width: 120px; width: 8%">
                        <template #body="slotProps">
                            <Tag 
                    ...
                    <Column field="schedule" header="SCH NO" style="min-width: 100px; width: 7%">
                        <template #body="slotProps">
                            <Link 
        ...
        <Dialog
            ...
            header="Print Preview - Salary Expenditure Ledger"
            :modal="true"
            :closable="true"
            ...
            <div class="print-content-wrapper" id="print-content-wrapper">
                <div class="print-content" id="print-content" ref="printContentRef">
                    ...
                    <div class="print-header">
                        ...
                        <h2 class="print-title">SALARY EXPENDITURE</h2>
...
const salaryTypeStats = computed(() => {
    const stats = {};
    props.entries.forEach(entry => {
        const type = entry.voucher_type || 'other';
        if (!stats[type]) {
            stats[type] = { type, total: 0, count: 0 };
        }
        stats[type].total += entry.amount || 0;
        stats[type].count++;
    });
...
const printContent = () => {
    try {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Salary Expenditure Ledger - ${monthName.value} ${selectedYear.value}</title>
                <meta charset="UTF-8">
                <style>
                    /* Reset */
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    ...
                    .print-ledger-table .pv-col { width: 9%; }
                    .print-ledger-table .mda-col { width: 11%; }
                    .print-ledger-table .voucher-type-col { width: 9%; }
                    .print-ledger-table .pay-point-col { width: 8%; }
                    .print-ledger-table .sch-col { width: 7%; }
                    .print-ledger-table .payee-col { width: 10%; }
                    .print-ledger-table .purpose-col { width: 13%; }
...
const applyFilters = () => {
    ...
    router.get('/expenditure-control/salary-ledger', params, {
        preserveState: true,
        preserveScroll: true,
        onFinish: () => {
            loading.value = false;
...
const exportExcel = () => {
    ...
    window.open(`/expenditure-control/salary-ledger/export?${params.toString()}`, '_blank');
